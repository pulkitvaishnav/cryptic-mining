...
def encrypt_CSM(inputFile):  # = sys.argv[1]):
    CSM = [[0, 0, 0]]
    i = 0
    for line in inputFile:
        i += 1
...
def decrypt_CSM(encrypted_CSM):
    for i in encrypted_CSM[1:]:
        i[2] = decrypt(i[0], i[1], i[2])
...
class JsonResponse(HttpResponse):
    ...
    def __init__(self, content={}, status=None,content_type='application/json'):
        super(JsonResponse, self).__init__(json.json.dumps(content),status=status, content_type=content_type)
